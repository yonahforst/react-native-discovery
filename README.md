...
You then need to add the Objective C part to your XCode project. Drag `DiscoveryReact.xcodeproj` from the `node_modules/react-native-ibeacon` folder into your XCode project. Click on the your project in XCode, goto Build Phases then Link Binary With Libraries and add `libDiscoveryReact.a` and `CoreLocation.framework`.

NOTE: Make sure you don't have the `DiscoveryReact` project open separately in XCode otherwise it won't work.
